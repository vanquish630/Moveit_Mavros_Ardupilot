...
bool ControllerBase::statusCheck()
{
    ...
    if (!conditionCheck(
        ...
        "Controller cannot generate command since mavros is not connected to apm",
    ...
    if (!conditionCheck(
        mavros_state_.mode == "GUIDED", 
        "Controller cannot generate command unless robot mode is set to GUIDED",
  ...
  mavros_moveit_controllers::SetOffboard::Response& res)
{
    ...
    return mavros_moveit_utils::setMavMode(mavros_state_, "GUIDED", set_mode_client_);
}
